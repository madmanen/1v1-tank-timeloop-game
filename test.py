...
Light_tank = Character(name="Light tank", health=80, speed=10, damage=20, fireRate=2, width=40, height=40, turn_speed=5) #Want its gun to be a shotgun
Medium_tank = Character(name="Medium tank", health=100, speed=20, damage=10, fireRate=5, width=50, height=50, turn_speed=6)
Heavy_tank = Character(name="Heavy tank", health=150, speed=10, damage=5, fireRate=8, width=60, height=60, turn_speed=2)
...
def draw():
    ...
    player_surf = pygame.Surface(player.size, pygame.SRCALPHA)
    pygame.draw.rect(player_surf, (0,255,0),player_surf.get_rect)
...
while True:
    ...
    if keys[pygame.K_LEFT]:
        player_angle +=Light_tank.turn_speed
    if keys[pygame.K_RIGHT]:
        player_angle -=Light_tank.turn_speed
    if keys[pygame.K_UP]:
        rad = math.radians(player_angle)
        player.x += Light_tank.speed * math.sin(rad)
        player.y += Light_tank.speed * math.cos(rad)
